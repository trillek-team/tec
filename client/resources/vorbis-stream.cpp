...
namespace tec {
	...
	VorbisStream::VorbisStream(std::size_t buffer_size) : buffer_size(buffer_size) {
	...
	std::size_t VorbisStream::BufferStream(ALint buffer) {
		...
		alBufferData(
			buffer, this->format, this->sbuffer, size * sizeof(ALshort), this->info.sample_rate);
	...
	void VorbisStream::Reset() {
		...
		this->totalSamplesLeft =
			stb_vorbis_stream_length_in_samples(this->stream) * this->info.channels;
	}
	...
	std::shared_ptr<VorbisStream> VorbisStream::Create(const FilePath& filename) {
		...
		// fopen to FileSystem ? Als we not are doing path valid or file existence check
		...
		if (stream->stream) {
			...
			stream->totalSamplesLeft =
				stb_vorbis_stream_length_in_samples(stream->stream) * stream->info.channels;
			SoundMap::Set(stream->GetName(), stream);
		}
		else {
			spdlog::get("console_log")
				->warn("[Vorbis-Stream] Can't load file {} code {} : {}", filename.toString(),
					error, VorbisErrorToString(error));
			stream.reset();
		}
		return stream;
	}
