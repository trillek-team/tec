...
namespace tec {
	...
	void AddInOutFunctors() {
		in_functors[GetTypeID<T>()] = [](const proto::Entity& entity,
										  const proto::Component& proto_comp) {
			T* comp = new T();
			comp->In(proto_comp);
			Multiton<eid, T*>::Set(entity.id(), comp);
		};
		update_functors[GetTypeID<T>()] =
			[](const proto::Entity& entity, const proto::Component& proto_comp, const state_id_t) {
				T* comp = new T();
				comp->In(proto_comp);
				Multiton<eid, T*>::Set(entity.id(), comp);
			};
	...
	void BuildTestEntities() {
		...
		auto deferred_shader_files = std::list<std::pair<Shader::ShaderType, FilePath>>{
			std::make_pair(
				Shader::VERTEX, FilePath::GetAssetPath("shaders/deferred_geometry.vert")),
			std::make_pair(
				Shader::FRAGMENT, FilePath::GetAssetPath("shaders/deferred_geometry.frag")),
		};
		...
		auto deferred_pl_shader_files = std::list<std::pair<Shader::ShaderType, FilePath>>{
			...
			std::make_pair(
				Shader::FRAGMENT, FilePath::GetAssetPath("shaders/deferred_pointlight.frag")),
		};
		auto deferred_pl_shader =
			Shader::CreateFromFile("deferred_pointlight", deferred_pl_shader_files);

		auto deferred_dl_shader_files = std::list<std::pair<Shader::ShaderType, FilePath>>{
			std::make_pair(Shader::VERTEX, FilePath::GetAssetPath("shaders/deferred_light.vert")),
			std::make_pair(
				Shader::FRAGMENT, FilePath::GetAssetPath("shaders/deferred_dirlight.frag")),
		};
		auto deferred_dl_shader =
			Shader::CreateFromFile("deferred_dirlight", deferred_dl_shader_files);

		auto deferred_stencil_shader_files = std::list<std::pair<Shader::ShaderType, FilePath>>{
			std::make_pair(Shader::VERTEX, FilePath::GetAssetPath("shaders/deferred_light.vert")),
		};
		auto deferred_stencil_shader =
			Shader::CreateFromFile("deferred_stencil", deferred_stencil_shader_files);
		...
		auto deferred_shadow_shader_files = std::list<std::pair<Shader::ShaderType, FilePath>>{
			...
			std::make_pair(
				Shader::FRAGMENT, FilePath::GetAssetPath("shaders/deferred_shadow.frag")),
		};
		auto deferred_shadow_shader =
			Shader::CreateFromFile("deferred_shadow", deferred_shadow_shader_files);

		{
			Entity bob(99);
			std::shared_ptr<MD5Mesh> mesh1 =
				MD5Mesh::Create(FilePath::GetAssetPath("bob/bob.md5mesh"));
			std::shared_ptr<MD5Anim> anim1 =
				MD5Anim::Create(FilePath::GetAssetPath("bob/bob.md5anim"), mesh1);
			Multiton<eid, Animation*>::Set(99, new Animation(anim1));
			// std::shared_ptr<ScriptFile> script1 = ScriptFile::Create("Script1",
			// FilePath::GetAssetPath("scripts/test.lua")); bob.Add<LuaScript>(script1);
		}
...

		{
			...
			VComputerCommand add_devoces([keybaord](VComputerSystem* vcomputer) {
				if (vcomputer->LoadROMFile(
						101, FilePath::GetAssetPath("asm/type1.ffi").toString())) {
		...
		if (fname.isValidPath() && fname.FileExists()) {
			...
			for (int i = 0; i < elist.entity_file_list_size(); i++) {
				...
				if (entity_filename.isValidPath() && entity_filename.FileExists()) {
					...
					_log->error("[ProtoLoadEntity] Error opening {} file. Can't find it",
