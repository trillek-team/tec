...
void SoundSystem::On(std::shared_ptr<EntityCreated> data) {
	...
	if (AudioSourceComponentMap::Has(entity_id)) {
		...
		case AUDIOSOURCE_STATE::PLAYING: alSourcePlay(source->source); break;
		case AUDIOSOURCE_STATE::PAUSED: alSourcePause(source->source); break;
		case AUDIOSOURCE_STATE::STOPPED: alSourceStop(source->source); break;
		}
	}
}
