...
namespace tec {
	...
	public:
		 ...
		 * modified by the OS and is just like typing into a text editor. \param[in] GLFWwindow*
		 * window \param[in] unsigned int uchar The Unicode character key code. \return void
		 */
		 ...
		 *
		 * \param[out] double* x, y The current x and y coordinate of the mouse in screen
		 * coordinates. \return void
		 */
	...
	private:
		 ...
		 * \param[in] const double x, y The new x and y coordinate of the mouse in screen
		 * coordinates. \return void
		 */
		...
		bool mouse_lock{
			false
		}; // If mouse lock is enabled causing the cursor to snap to mid-window each movement event.
	};
} // namespace tec
