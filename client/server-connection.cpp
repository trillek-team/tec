...
namespace tec { namespace networking {
	...
	ServerConnection::ServerConnection() : socket(io_service) {
		...
		RegisterMessageHandler(MessageType::SYNC,
			[this](const ServerMessage& message) { this->SyncHandler(message); });
	...
	void ServerConnection::Send(ServerMessage& msg) {
		try {
			...
			std::cerr << "ServerConnection::Send(ServerMessage&): asio::write:" << e.what()
					  << std::endl;
		}
	}
	...
	void ServerConnection::StartRead() {
		...
		while (!this->stopped) {
			try {
				...
				std::cerr << "ServerConnection::StartRead(): Error reading header:" << e.what()
						  << std::endl;
			}
	...
	void ServerConnection::GameStateUpdateHandler(const ServerMessage& message) {
		...
		if (recv_state_id <= this->last_received_state_id) {
			...
			next_state.In(gsu);
			std::shared_ptr<NewGameStateEvent> new_game_state_msg =
				std::make_shared<NewGameStateEvent>();
