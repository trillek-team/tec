...
void Material::SetPolygonMode(const GLenum mode) {
	...
	case GL_POINT:
	case GL_LINE:
	case GL_FILL: this->polygon_mode = mode; break;
	default: this->polygon_mode = GL_FILL;
	}
}
...
void Material::SetDrawElementsMode(const GLenum mode) {
	...
	case GL_TRIANGLES:
	case GL_PATCHES: this->draw_elements_mode = mode; break;
	default: this->draw_elements_mode = GL_TRIANGLES;
	}
}

const GLenum Material::GetDrawElementsMode() const { return this->draw_elements_mode; }

void Material::AddTexture(std::shared_ptr<TextureObject> tex) { this->textures.push_back(tex); }
