...
namespace tec {
	...
	void FilePath::NormalizePath() {
		...
		if (path.size() > 2 && std::isalpha(path.at(0)) &&
			path.at(1) == ':') { // x: windows unit to remove
	...
	bool FilePath::isValidPath() const {
		if (!this->isAbsolutePath()) { // No contains drive, so check if is a valid relative path
			...
			if (path.size() >= 3 && path.at(0) == '.' &&
				(path.at(1) == PATH_SEPARATOR_C ||
					(path.at(1) == '.' && path.at(2) == PATH_SEPARATOR_C))) {
				return true;
			}
			else {
				return false;
			...
			if (tmp.DirExists()) {
				...
				if (tmp.DirExists()) {
					...
					if (tmp.DirExists()) {
						...
						tmp = FilePath::GetProgramPath().BasePath().BasePath() /
							  ("share/" + app_name + "/assets/");
						if (tmp.DirExists()) {
							FilePath::assets_base = tmp.toString();
						}
					}
