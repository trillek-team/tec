...
namespace tec {
template <typename T> class EventSystem final {
private:
	EventSystem() {}
	EventSystem(const EventSystem& right) { instance = right.instance; }
...
public:
	static std::shared_ptr<EventSystem<T>> Get() {
		std::call_once(EventSystem::only_one, []() { EventSystem<T>::instance.reset(new EventSystem<T>()); });
	...
	* \return void
	*/
	void Subscribe(EventQueue<T>* subscriber) { this->subscribers[0].push_back(subscriber); }
...
template <typename T> std::once_flag EventSystem<T>::only_one;

template <typename T> std::shared_ptr<EventSystem<T>> EventSystem<T>::instance = nullptr;
} // namespace tec
