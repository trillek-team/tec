...
namespace tec {
		...
		if (this->server_states.size() >= 2) {
			interpolation_accumulator += delta_time;
			{
				if (interpolation_accumulator >= INTERPOLATION_RATE) {
					...
					if (this->client_id != 0) {
						this->predictions.emplace(std::make_pair(
							this->command_id, this->interpolated_state.positions[this->client_id]));
					}
					...
					if (this->client_id != 0) {
						this->base_state.positions[this->client_id] =
							this->predictions.at(this->command_id);
						this->interpolated_state.positions[this->client_id] =
							this->predictions.at(this->command_id);
					}
					...
					for (auto orientation : to_state.orientations) {
						...
						this->interpolated_state.orientations[orientation.first] =
							orientation.second;
					}
			...
			}
			{
				...
				float lerp_percent = static_cast<float>(
					...
					(INTERPOLATION_RATE * (to_state.state_id - this->base_state.state_id)));
				if (lerp_percent > 0.0) {
					for (auto position : to_state.positions) {
						if (this->base_state.positions.find(position.first) !=
							this->base_state.positions.end()) {
							this->interpolated_state.positions[position.first].value =
								glm::lerp(base_state.positions.at(position.first).value,
									position.second.value, lerp_percent);
						}
						else {
							this->interpolated_state.positions[position.first] = position.second;
						}
					...
					for (auto velocity : to_state.velocities) {
						if (this->base_state.velocities.find(velocity.first) !=
							this->base_state.velocities.end()) {
							this->interpolated_state.velocities[velocity.first].linear =
								glm::lerp(base_state.velocities.at(velocity.first).linear,
									velocity.second.linear, lerp_percent);
							this->interpolated_state.velocities[velocity.first].angular =
								glm::lerp(base_state.velocities.at(velocity.first).angular,
									velocity.second.angular, lerp_percent);
						}
						else {
							this->interpolated_state.velocities[velocity.first].linear =
								velocity.second.linear;
							this->interpolated_state.velocities[velocity.first].angular =
								velocity.second.angular;
						}
					}
					for (auto orientation : to_state.orientations) {
						if (this->base_state.orientations.find(orientation.first) !=
							this->base_state.orientations.end()) {
							this->interpolated_state.orientations[orientation.first].value =
								glm::slerp(base_state.orientations.at(orientation.first).value,
									orientation.second.value, lerp_percent);
						}
						else {
							this->interpolated_state.orientations[orientation.first] =
								orientation.second;
	...
	void GameStateQueue::QueueServerState(GameState&& new_state) {
		if (new_state.state_id > this->last_server_state_id) {
			this->server_states_array[server_state_array_index % SERVER_STATES_ARRAY_SIZE] =
				new_state;
	...
	void GameStateQueue::CheckPredictionResult(GameState& new_state) {
		if (this->client_id != 0) {
			std::cout << "Client command id: " << this->command_id
					  << " server ack: " << new_state.command_id << std::endl;
			...
			if (this->predictions.find(new_state.command_id) != this->predictions.end()) {
				if (new_state.positions.find(this->client_id) != new_state.positions.end()) {
					auto dif = new_state.positions[this->client_id].value -
							   this->predictions[new_state.state_id].value;
					std::cout << "Diff (" << dif.x << ", " << dif.y << ", " << dif.z
							  << ") <> Predictions size = " << this->predictions.size()
							  << std::endl;
				}
