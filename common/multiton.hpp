...
*/
template <typename ID_T, typename T> class Multiton {
public:
	static typename std::map<ID_T, T>::iterator Begin() { return instances.begin(); }

	static typename std::map<ID_T, T>::iterator End() { return instances.end(); }

	static typename std::size_t Size() { return instances.size(); }
	...
	*/
	...
	static bool Has(const ID_T id) { return instances.find(id) != instances.end(); }
	...
	* \return void
	*/
	static void Set(const ID_T id, T instance) { instances[id] = instance; }
...
template <typename ID_T, typename T> std::map<ID_T, T> Multiton<ID_T, T>::instances;

template <typename ID_T, typename T> T Multiton<ID_T, T>::default_value;
} // namespace tec
