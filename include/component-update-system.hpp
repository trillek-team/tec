...
namespace tec {
	...
	struct ComponentUpdateList {
		...
		state_id_t frame = 0; // The frame id the changes are in.
	...
	public:
		...
		static void UpdateAll(state_id_t frame_id) {
		...
		static void RegisterUpdateFunction(std::function<void(state_id_t)>&& update_func) {
			update_funcs.push_back(update_func);
		}
	private:
		static std::list<std::function<void(state_id_t)>> update_funcs;
	};
	...
	public:
		...
		static void Initialize() {
			auto update_func = [ ] (state_id_t frame_id) {
		...
		static void UpdateTo(state_id_t frame_id) {
			{
		...
		static void SubmitUpdate(eid entity_id, std::shared_ptr<T> value, state_id_t frame_id = 0) {
			std::lock_guard<std::mutex> list_lock(list_mutex);
		...
		static void SubmitRemoval(eid entity_id, state_id_t frame_id = 0) {
			std::lock_guard<std::mutex> list_lock(list_mutex);
			ComponentUpdateList<T>* list = nullptr;
			auto it = future_update_lists.begin();
			if (frame_id > base_frame_id) {
	...
	protected:
		...
		static state_id_t base_frame_id;
	};
	...
	template <typename T>
	state_id_t ComponentUpdateSystem<T>::base_frame_id = -1;
