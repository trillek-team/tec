...
namespace tec { namespace networking {
	...
	void ClientConnection::DoJoin() {
		// Build an entity
		Entity self(this->id);
		self.Add<Position, Orientation, Velocity, View>(
			glm::vec3(5, 1.0, 5), glm::vec3(.5f, 0.f, 0.f), Velocity(), true);
		...
		this->entity.SerializeToArray(
			entity_create_msg.GetBodyPTR(), static_cast<int>(entity_create_msg.GetBodyLength()));
					...
					switch (this->current_read_msg.GetMessageType()) {
						case MessageType::CHAT_MESSAGE:
							...
							std::cout.write(this->current_read_msg.GetBodyPTR(),
								this->current_read_msg.GetBodyLength())
								<< std::endl;
							break;
						...
						case MessageType::CLIENT_COMMAND: {
							...
							std::shared_ptr<ClientCommandsEvent> data =
								std::make_shared<ClientCommandsEvent>();
	...
	void ClientConnection::UpdateGameState(const GameState& full_state) {
		for (auto pair : full_state.positions) {
			if (full_state.positions.find(pair.first) != full_state.positions.end()) {
				this->state_changes_since_confirmed.positions[pair.first] =
					full_state.positions.at(pair.first);
			}
			if (full_state.orientations.find(pair.first) != full_state.orientations.end()) {
				this->state_changes_since_confirmed.orientations[pair.first] =
					full_state.orientations.at(pair.first);
			}
			if (full_state.velocities.find(pair.first) != full_state.velocities.end()) {
				this->state_changes_since_confirmed.velocities[pair.first] =
					full_state.velocities.at(pair.first);
			}
		}
	}

	tec::networking::ServerMessage ClientConnection::PrepareGameStateUpdateMessage(
		...
		for (auto pos : this->state_changes_since_confirmed.positions) {
			...
			if (this->state_changes_since_confirmed.orientations.find(pos.first) !=
				this->state_changes_since_confirmed.orientations.end()) {
				tec::Orientation ori =
					this->state_changes_since_confirmed.orientations.at(pos.first);
