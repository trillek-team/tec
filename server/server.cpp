...
namespace tec { namespace networking {
	...
	Server::Server(tcp::endpoint& endpoint) : acceptor(io_service, endpoint), socket(io_service) {
		...
		memcpy(
			this->greeting_msg.GetBodyPTR(), message.c_str(), this->greeting_msg.GetBodyLength());
		...
		acceptor.async_accept(socket, [this](std::error_code error) {
			...
			if (!error) {
				...
				std::shared_ptr<ClientConnection> client =
					std::make_shared<ClientConnection>(std::move(socket), this);

				// self_protopack does contain a renderable component
				static FilePath others_protopack =
					FilePath::GetAssetPath("protopacks/others.proto");
				...
				for (auto other_client : clients) {
					...
					other_entity.SerializeToArray(other_client_entity_msg.GetBodyPTR(),
				...
				for (auto entity : entities) {
					...
					entity.second.SerializeToArray(other_client_entity_msg.GetBodyPTR(),
						static_cast<int>(other_client_entity_msg.GetBodyLength()));
					other_client_entity_msg.SetMessageType(MessageType::ENTITY_CREATE);
					other_client_entity_msg.encode_header();
